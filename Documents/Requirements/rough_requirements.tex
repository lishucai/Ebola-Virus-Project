...
\section{Specific requirements}
	\begin{enumerate}
		\item Our program will be able take the thermal image, and isolate the head of the person to be analysized.
		\item The program will analyze the pixels in the image and produce an average temperature of the skin.
		\item The mathematical model should take the observed skin temperature calculated from the camera and predict a core body temperature.
		\item The whole process of taking the picture, and analyzing the image should take less than one minute.
		\item Our program will have at most a 40\% false negative rate.
		\item Our program should provide a simple yes or no to whether or not the person's core body temperature is elevated.
		\item Our final product will be a program that takes in an input of text file or two inputs, temperature taken from the thermal camera, and the real temperature taken from the patients ear.
			  It will then output in a textfile the if the result of the measured and real temperature pair is accurate.
	\end{enumerate}
